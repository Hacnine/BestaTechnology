...
interface CadConsumptionSectionEditProps {
  data: any;
...
const CadConsumptionSectionEdit = ({ data, onChange }: CadConsumptionSectionEditProps) => {
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      // When incoming data does not include client-side ids, generate them
      setRows(
        data.map((row: any, index: number) => ({
          id: row.id || `cad-${Date.now()}-${index}`,
...
export default CadConsumptionSectionEdit;

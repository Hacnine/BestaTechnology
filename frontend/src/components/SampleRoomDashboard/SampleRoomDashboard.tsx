...
import {
  ...
  const SampleDevelopement = () => {
    ...
    const { data, isLoading: isTableLoading, error: tableError } = useGetSampleDevelopmentsQuery({
      page,
      pageSize,
    ...
    const showActualCompleteColumn = (data?.data || []).some((r: any) => !!(
      r.actualSampleCompleteDate || r.actualCompleteDate || r.actual_sample_complete_date
    ));
              ...
              <TableBody>
                {isTableLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={colCount} className="text-center py-8">
                      <div className="text-red-500">
                        <p className="font-medium">Failed to load Sample Developments</p>
                    ...
                    <TableCell colSpan={colCount} className="text-center py-12">
                      <div className="flex flex-col items-center justify-center space-y-3">
                        ...
                        <div className="text-muted-foreground">
                          <p className="font-medium text-lg">No Sample Development data found</p>
                  ...
                  (data?.data || []).map((row: any, index: number) => {
                    ...
                    const { sampleRemaining, sampleActualBadge } = calculateTnaValues({
                      id: row.id,
                      sampleDevelopment: {
                        sampleCompleteDate: row.sampleCompleteDate,
                    ...
                    return (
                      <TableRow key={row.id} className="animate__animated animate__fadeInUp" style={{ animationDelay: `${index * 0.05}s` }}>
                        ...
                        <TableCell><span className="text-sky-500 font-medium ">{row.samplemanName ? `Accepted By ${row.samplemanName}` : "Not Assigned"}</span></TableCell>
                        <TableCell>
                          {row.sampleReceiveDate
                            ? new Date(row.sampleReceiveDate).toLocaleDateString('en-US', {
                                year: 'numeric',
                                month: '2-digit',
                                day: '2-digit'
                              })
                        ...
                        <TableCell>
                          {sampleActualBadge ? (
                            ...
                            <span className={sampleRemaining < 0 ? "text-red-500" : "text-blue-600"}>
                              {sampleRemaining < 0
                            ...
                            row.sampleCompleteDate
                              ? new Date(row.sampleCompleteDate).toLocaleDateString('en-US', {
                                  year: 'numeric',
                                  month: '2-digit',
                                  day: '2-digit'
                                })
                        ...
                        {showActualCompleteColumn && (
                          <TableCell>
                            {row.actualSampleCompleteDate
                              ? new Date(row.actualSampleCompleteDate).toLocaleDateString('en-US', {
                                  year: 'numeric',
                                  month: '2-digit',
                                  day: '2-digit'
                                })
